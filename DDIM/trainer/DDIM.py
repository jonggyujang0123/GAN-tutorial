...
class DDIM():
    ...
    def p_sample_loop(self, shape):
        ...
        times = torch.linspace(0., self.cfg.timesteps, steps= self.cfg.sampling_timesteps+2)[:-1] #(start, end)
        times= list(reversed(times.int().tolist()))
        time_pairs = list(zip(times[:-1], times[1:]))
        img = torch.randn(shape, device= device)
        imgs = []

        tqdm_time = tqdm(time_pairs,
                desc = 'sampling loop time step',
                disable = self.args.local_rank not in [-1,0])
        for time, time_next in tqdm_time:
            alpha = self.alphas_comprod_prev[time]
            alpha_next = self.alphas_comprod_prev[time_next]

            time_cond = torch.full((b,), time, device= self.cfg.device, dtype = torch.long)
            pred_noise  = self.model(img,time_cond)
            x_start =  (img - (1-alpha).sqrt()*pred_noise) / alpha.sqrt()

            if self.cfg.clip_denoised:
                x_start.clamp_(-1., 1.)

            sigma = self.cfg.eta * ((1 - alpha / alpha_next)*(1-alpha_next) / (1-alpha)).sqrt()


            pred_noise_coeff = (1-alpha_next-sigma**2).sqrt()
            noise = torch.randn_like(x_start)
            img = alpha_next.sqrt() * x_start + \
    ...
    #    print("Checkpoint saved successfully to '{}' at (self.epoch {})\n"
    ...
    def train(self):
        ...
        self.model.train()
        Loss = AverageMeter()
        for self.epoch in range(start_epoch, self.cfg.epochs):
            ...
            for data in tepoch:
                ...
                if self.args.local_rank in [-1,0]:
                    ...
                    tepoch.set_description(
                            f'Epoch {self.epoch}: loss is {Loss.avg:2.5f}, lr is {self.scheduler.get_lr()[0]:.2E}'
                            )
            self.test()
            if self.args.local_rank in [-1,0]:
